...
import { getOrderList, getOrderDetail, type ShopeeOrderDetail } from './api.client';

const ORDER_DETAIL_CHUNK_SIZE = 50; // limit get_order_detail: order_sn_list maks 50
...
// Shopee sync sedang di-hold (partner key belum tersedia) — mapping status
// di bawah belum divalidasi terhadap partner key nyata.
const SHOPEE_STATUS_MAP: Record<string, NormalizedOrder['status']> = {
  UNPAID: 'new',
...
function mapOrder(order: ShopeeOrderDetail): NormalizedOrder {
  return {
...
export const shopeeAdapter: PlatformAdapter = {
  ...
  exchangeCodeForToken: async (code, shopIdExternal) => {
    if (!shopIdExternal) throw new Error('Shopee callback butuh shop_id');
    return auth.exchangeCodeForToken(code, shopIdExternal);
  },
  ...
  fetchRecentOrders: async (creds, sinceSeconds) => {
    ...
    while (more) {
      ...
      if (listResult.error) throw new Error(`Shopee get_order_list error: ${listResult.error} - ${listResult.message}`);
